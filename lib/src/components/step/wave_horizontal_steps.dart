...
class WaveHorizontalSteps extends StatefulWidget {
  /// The steps of the stepper whose titles, subtitles, icons always get shown.
  ///
  /// 步骤条中元素的列表
  final List<WaveUIStep> steps;
  ...
  @override
  State<StatefulWidget> createState() => WaveHorizontalStepsState();
}

class WaveHorizontalStepsState extends State<WaveHorizontalSteps> {
  Color get _primary {
  ...
  @override
  void didUpdateWidget(covariant WaveHorizontalSteps oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final List<WaveUIStep> steps = widget.steps;
  ...
  Widget _applyStepItem(WaveUIStep step, int index) {
    return Expanded(
      child: Column(
  ...
  Widget _applyStepAndLine(WaveUIStep step, int index) {
    return Row(
  ...
  Widget _applyStepIcon(WaveUIStep step, int index) {
    Widget icon;
    ...
    if (step.state != null) {
      switch (step.state) {
        case WaveUIStepState.indexed:
          icon = _getIndexIcon(index);
          break;
        case WaveUIStepState.complete:
          icon = _getCompletedIcon(step);
          break;
        case WaveUIStepState.doing:
  ...
  Widget _getIndexIcon(int index) {
    ...
    switch (index) {
      case 1:
        icon = WaveUITools.getAssetSizeImage(WaveAsset.iconStep2, 20, 20);
        break;
      case 2:
        icon = WaveUITools.getAssetSizeImage(WaveAsset.iconStep3, 20, 20);
        break;
      case 3:
        icon = WaveUITools.getAssetSizeImage(WaveAsset.iconStep4, 20, 20);
        break;
      case 4:
        icon = WaveUITools.getAssetSizeImage(WaveAsset.iconStep5, 20, 20);
        break;
      default:
        icon = WaveUITools.getAssetSizeImage(WaveAsset.iconStepDoing, 20, 20);
        break;
    }
    return icon;
  }

  Widget _applyStepContent(WaveUIStep step, int index) {
  ...
  Widget _getCompletedIcon(WaveUIStep step) {
    ...
    /// 使用组件默认的icon
    return WaveUITools.getAssetSizeImage(WaveAsset.iconStepCompleted, 20, 20,
        color: _primary);
  }

  Widget _getDoingIcon(WaveUIStep step) {
    ...
    // 使用组件默认的icon
    return WaveUITools.getAssetSizeImage(WaveAsset.iconStepDoing, 20, 20,
        color: _primary);
  }
}

enum WaveUIStepState {
...
class WaveUIStep {
  ...
  final WaveUIStepState? state;
}

class WaveStepsController with ChangeNotifier {
  /// 指示当前进行态的步骤
  int currentIndex;
