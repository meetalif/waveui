...
/// /// /// /// /// /// /// /// /// /
class WaveAbnormalStateUtils {
  /// 通过状态获取对应空页面widget
  /// status: 页面状态类型为[EmptyState]
  static Widget getEmptyWidgetByState(
      BuildContext context, AbnormalState status,
      {Image? img, WaveEmptyStatusIndexedActionClickCallback? action}) {
    if (AbnormalState.getDataFailed == status) {
      return WaveAbnormalStateWidget(
        img: img ?? WaveUITools.getAssetImage(WaveAsset.noData),
        title: WaveIntl.of(context).localizedResource.fetchErrorAndRetry,
        operateTexts: <String>[
          WaveIntl.of(context).localizedResource.clickPageAndRetry
        ],
        action: action,
      );
    } else if (AbnormalState.networkConnectError == status) {
      return WaveAbnormalStateWidget(
        img: img ?? WaveUITools.getAssetImage(WaveAsset.networkError),
        title: WaveIntl.of(context).localizedResource.netErrorAndRetryLater,
        operateTexts: <String>[
          WaveIntl.of(context).localizedResource.clickPageAndRetry
        ],
        action: action,
...
/// 空页面操作区域按钮的点击回调
/// index: 被点击按钮的索引
typedef WaveEmptyStatusIndexedActionClickCallback = void Function(int index);

/// 异常页面展示一般用于网络错误、数据为空的提示和引导
// ignore: must_be_immutable
class WaveAbnormalStateWidget extends StatelessWidget {
  /// 图片
  final Image? img;
  ...
  /// 点击事件回调
  final WaveEmptyStatusIndexedActionClickCallback? action;
