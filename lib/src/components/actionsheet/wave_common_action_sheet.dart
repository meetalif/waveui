...
typedef WaveCommonActionSheetItemClickCallBack = void Function(
    int index, WaveCommonActionSheetItem actionItem);
typedef WaveCommonActionSheetItemClickInterceptor = bool Function(
    int index, WaveCommonActionSheetItem actionItem);

/// 每行样式
enum WaveCommonActionSheetItemStyle {
  /// 默认样式
  normal,

  /// 链接样式，颜色使用主题色号[WaveGet.theme.colorScheme.primary]
  link,

  /// 警示项 ，颜色使用[WaveCommonConfig.brandError]
  alert,
}

class WaveCommonActionSheetItem {
  /// 标题文字
  String title;
  ...
  /// 样式 [WaveActionSheetActionStyle]
  final WaveCommonActionSheetItemStyle actionStyle;
  ...
  WaveCommonActionSheetItem(
    this.title, {
    this.desc,
    this.actionStyle = WaveCommonActionSheetItemStyle.normal,
    this.titleStyle,
    this.descStyle,
  });
}
...
/// 吸底列表弹框，可自定义标题文案
/// 可通过配置[WaveCommonActionSheetItemStyle]来设定 item 的样式
// ignore: must_be_immutable
class WaveCommonActionSheet extends StatelessWidget {
  /// 每个选项相关的配置信息的列表
  /// 每个选项支持修改内容见[WaveCommonActionSheetItem]
  final List<WaveCommonActionSheetItem> actions;
  ...
  /// Action Item 的点击事件
  final WaveCommonActionSheetItemClickCallBack? clickCallBack;

  /// Action Item 点击事件拦截回调
  final WaveCommonActionSheetItemClickInterceptor? onItemClickInterceptor;
  ...
  // 配置每个选项内部信息
  // action 每个item配置项 [WaveCommonActionSheetItem]
  Widget _configTile(WaveCommonActionSheetItem action) {
