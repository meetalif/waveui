...
typedef WaveItemTitleBuilder<T> = dynamic Function(int index, T entity);
typedef WaveItemDeleteCallback<T> = bool Function(
    int deleteIdx, T deleteEntity);
typedef WaveListDismissCallback = void Function(bool isClosedByClearButton);

/// 监听数据刷新和列表关闭操作
class WaveSelectedListActionSheetController extends ChangeNotifier {
  /// 是否刷新数据
...
class WaveSelectedListActionSheet<T> {
  ...
  final WaveItemTitleBuilder<T> itemTitleBuilder;

  /// 控制视图隐藏/刷新列表等方法
  final WaveSelectedListActionSheetController? controller;
  ...
  final WaveItemDeleteCallback<T>? onItemDelete;

  /// 视图显示时的回调
  final VoidCallback? onListShowed;

  /// 视图隐藏时的回调，会把是否是清空按钮触发的销毁视图回传
  final WaveListDismissCallback? onListDismissed;
  ...
  void _innerShow(bool showByKey) {
    ...
    WaveSelectedListActionSheetController? tempController = controller;
    if (tempController == null) {
      tempController = WaveSelectedListActionSheetController();
      tempController._isHidden = false;
    }
    _WaveActionSheetSelectedItemListContentWidget content =
        _WaveActionSheetSelectedItemListContentWidget<T>(
      itemWidget: this,
      onDismiss: (isClear) {
...
class _WaveActionSheetSelectedItemListContentWidget<T> extends StatefulWidget {
  final WaveSelectedListActionSheet itemWidget;
  ...
  _WaveActionSheetSelectedItemListContentWidget(
  ...
  State<StatefulWidget> createState() =>
      _WaveActionSheetSelectedItemListState<T>();
}

class _WaveActionSheetSelectedItemListState<T>
    extends State<_WaveActionSheetSelectedItemListContentWidget<T?>>
    with TickerProviderStateMixin {
  bool _isClosedByClear = false;
  WaveSelectedListActionSheetController? _controller;
  ...
  void onClearAction() {
    if (widget.itemWidget.onClear == null) {
      ...
      WaveDialogManager.showConfirmDialog(context,
          title:
              WaveIntl.of(context).localizedResource.confirmClearSelectedList,
          cancel: WaveIntl.of(context).localizedResource.cancel,
          confirm: WaveIntl.of(context).localizedResource.ok, onConfirm: () {
  ...
  Widget build(BuildContext context) {
    ...
    String title =
        (widget.itemWidget.title != null && widget.itemWidget.title!.isNotEmpty)
            ...
            : WaveIntl.of(context).localizedResource.selectedList;
