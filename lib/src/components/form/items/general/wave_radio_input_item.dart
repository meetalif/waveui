...
class WaveRadioInputFormItem extends StatefulWidget {
  /// 录入项的唯一标识，主要用于录入类型页面框架中
  final String? label;

  /// 录入项类型，主要用于录入类型页面框架中
  final String type = WaveInputItemType.radioInputType;
  ...
  /// 选项选中状态变化回调
  final OnWaveFormRadioValueChanged? onChanged;
  ...
  @override
  WaveRadioInputFormItemState createState() {
    return WaveRadioInputFormItemState();
  }
}
...
class WaveRadioInputFormItemState extends State<WaveRadioInputFormItem> {
    ...
    if (options != null && options.isNotEmpty) {
      ...
      for (String? item in options) {
        ...
        double optionWidth = painter.width +
            WaveFormUtil.optionsMiddlePadding(widget.themeData!)!.left +
  ...
  Widget _buildTitleTextWidget() {
    return Text(widget.title,
        ...
        style: WaveFormUtil.getTitleTextStyle(widget.themeData!));
  }
  ...
  List<Widget> getRadioList(List<String>? options) {
    ...
    for (int index = 0; index < options.length; ++index) {
      ...
      result.add(
        Container(
          padding: WaveFormUtil.optionsMiddlePadding(widget.themeData!),
          child: Row(
            children: <Widget>[
              WaveRadioButton(
                ...
                onValueChangedAtIndex: (int position, bool selected) {
                  ...
                  WaveFormUtil.notifyRadioStatusChanged(
