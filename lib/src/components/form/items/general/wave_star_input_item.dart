...
class WaveStarsFormItem extends StatefulWidget {
  /// 录入项的唯一标识，主要用于录入类型页面框架中
  final String? label;

  /// 录入项类型，主要用于录入类型页面框架中
  String type = WaveInputItemType.starInputType;
  ...
  @override
  WaveStarsFormItemState createState() {
    return WaveStarsFormItemState();
  }
}
...
class WaveStarsFormItemState extends State<WaveStarsFormItem> {
  ...
  Image getStar(int index, int selectCount, int sum) {
    if (selectCount <= 0) {
      return WaveUITools.getAssetImage(WaveAsset.iconStarUnSelect);
    }

    if (index < selectCount) {
      return WaveUITools.getAssetImage(WaveAsset.iconStarSelect);
    }
