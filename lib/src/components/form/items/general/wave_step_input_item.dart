...
class WaveStepInputFormItem extends StatefulWidget {
  /// 录入项的唯一标识，主要用于录入类型页面框架中
  final String? label;

  /// 录入项类型，主要用于录入类型页面框架中
  final String type = WaveInputItemType.textStepInputType;
  ...
  /// 当前值变化回调
  final OnWaveFormValueChanged? onChanged;
  ...
  @override
  WaveStepInputFormItemState createState() {
    return WaveStepInputFormItemState();
  }
}

class WaveStepInputFormItemState extends State<WaveStepInputFormItem> {
  ...
  Image _getAddIcon() {
    if (!widget.isEdit) {
      return WaveUITools.getAssetImage(WaveAsset.iconAddDisable);
    }
    ...
    return WaveUITools.getAssetImage(WaveAsset.iconAddEnable);
  }
  ...
  Image _getMinusIcon() {
    if (!widget.isEdit) {
      return WaveUITools.getAssetImage(WaveAsset.iconMinusDisable);
    }
    ...
    return WaveUITools.getAssetImage(WaveAsset.iconMinusEnable);
  }
