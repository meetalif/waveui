...
class WaveGeneralFormItem extends StatefulWidget {
  /// 录入项的唯一标识，主要用于录入类型页面框架中
  final String? label;

  /// 录入项类型，主要用于录入类型页面框架中
  final String type = WaveInputItemType.labelGeneralType;
  ...
  State<StatefulWidget> createState() {
    return WaveGeneralFormItemState();
  }
}

class WaveGeneralFormItemState extends State<WaveGeneralFormItem> {
  @override
                      ...
                      child: Container(
                        ...
                        child: GestureDetector(
                          onTap: () {
                            ...
                            if (WavePrefixIconType.add ==
                                widget.prefixIconType) {
                              if (widget.onAddTap != null) {
                                widget.onAddTap!();
                              }
                            } else if (WavePrefixIconType.remove ==
                                widget.prefixIconType) {
                          ...
                          child:
                              WaveFormUtil.getPrefixIcon(widget.prefixIconType),
                        ),
                      ),
                    ),
                    Offstage(
            ...
            child: Container(
                ...
                child: widget.subTitleWidget ??
                    Text(
                      widget.subTitle ?? "",
                      style:
                          WaveFormUtil.getSubTitleTextStyle(widget.themeData!),
                    )),
          ),
          Offstage(
              offstage: (widget.error.isEmpty),
              child: Container(
                  padding: WaveFormUtil.computeErrorEdgeInsets(
