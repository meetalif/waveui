...
import 'package:waveui/src/components/picker/time_picker/date_range_picker/wave_date_range_picker.dart';
...
class DateTimeFormatter {
  static DateTime? convertStringToDate(String? format, String? date) {
    if (WaveUITools.isEmpty(format) || WaveUITools.isEmpty(date)) return null;
  ...
  static String generateDateFormat(
      String? dateFormat, WaveDateTimePickerMode pickerMode) {
  ...
  static String generateDateRangePickerFormat(
      String? dateFormat, WaveDateTimeRangePickerMode pickerMode) {
    if (dateFormat != null && dateFormat.isNotEmpty) {
      return dateFormat;
    }
    switch (pickerMode) {
  ...
  static String _formatMonth(int value, String format) {
    List<String> months = WaveIntl.currentResource.months;
  ...
  static String _formatWeek(int value, String format) {
    if (format.contains('EEEE')) {
      ...
      List<String> weeks = WaveIntl.currentResource.weekFullName;
      return format.replaceAll('EEEE', weeks[value - 1]);
    }
    // EEE: the short name of week, e.g. Mon
    List<String> weeks = WaveIntl.currentResource.weekShortName;
