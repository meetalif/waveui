import 'package:waveui/src/components/picker/base/wave_picker.dart';
...
import 'base/wave_picker_constants.dart';
...
typedef WaveMultiDataPickerCreateWidgetCallback = Widget Function(
...
/// 数据适配 Delegate
abstract class WaveMultiDataPickerDelegate {
  /// 定义显示几列内容
...
class WaveMultiDataPicker extends StatefulWidget {
  /// 多级数据选择弹窗标题
  final String title;
  ...
  final WaveMultiDataPickerDelegate delegate;
  ...
  final WaveMultiDataPickerCreateWidgetCallback? createItemWidget;
  ...
  @override
  _WaveMultiDataPickerState createState() => _WaveMultiDataPickerState();
...
/// 实现了部分默认逻辑的 Delegate
class WaveDefaultMultiDataPickerDelegate
    implements WaveMultiDataPickerDelegate {
  ///数据源
  List<WaveMultiDataPickerEntity> data;
  ...
  WaveDefaultMultiDataPickerDelegate(
      {required this.data,
    ...
    if (data.isNotEmpty) {
      ...
      for (WaveMultiDataPickerEntity wavePickerItem in data) {
        if (wavePickerItem.children.isNotEmpty) {
          _numberOfComponent = 2;

          for (WaveMultiDataPickerEntity wavePickerItem1
              in wavePickerItem.children) {
            if (wavePickerItem1.children.isNotEmpty) {
  ...
  int numberOfRowsInComponent(int component) {
    ...
    if (0 == component) {
      ...
      List<WaveMultiDataPickerEntity> secondMap =
  ...
  String titleForRowInComponent(int component, int index) {
    if (0 == component) {
      ...
      WaveMultiDataPickerEntity wavePickerItem = data[firstSelectedIndex];
      List<WaveMultiDataPickerEntity> secondList = wavePickerItem.children;
      return secondList[index].text;
    } else {
      WaveMultiDataPickerEntity wavePickerItem = data[firstSelectedIndex];
      List<WaveMultiDataPickerEntity> secondList = wavePickerItem.children;
      List<WaveMultiDataPickerEntity> threeList =
...
/// 适用于 WaveDefaultMultiDataPickerDelegate 的数据类
class WaveMultiDataPickerEntity {
  /// 显示内容
  final String text;
  ...
  /// 子项
  final List<WaveMultiDataPickerEntity> children;

  WaveMultiDataPickerEntity({
    required this.text,
    this.value,
    this.children = const [],
  });
