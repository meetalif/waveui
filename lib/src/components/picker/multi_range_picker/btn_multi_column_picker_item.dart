...
class WaveMultiRangePickerCommonItem extends StatelessWidget {
  final WavePickerEntity item;
  ...
  String _getSelectedItemCount(WavePickerEntity item) {
    String itemCount = "";
    if ((WaveMultiColumnPickerUtil.getTotalColumnCount(item) < 3 ||
        !isFirstLevel)) {
      int count =
  ...
  bool _hasCheckBoxBrother(WavePickerEntity item) {
    int? count;
    if (item.parent != null) {
