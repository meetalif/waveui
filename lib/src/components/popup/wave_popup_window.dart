...
class WavePopupWindow extends StatefulWidget {
  /// 依附的组件的 Context
  ...
  /// 依附的组件和 WavePopUpWindow 组件共同持有的 GlobalKey
  ...
  /// popUpWindow 位于 targetView 的方向，默认为 [WavePopupDirection.bottom]
  final WavePopupDirection popDirection;
  ...
  /// 显示 popUpWindow
  /// [text] 显示的文本内容
  /// [popKey] 依附的组件和 WavePopUpWindow 组件共同持有的 GlobalKey
  /// [popDirection] 箭头的方向
  ...
  static void showPopWindow(context, String? text, GlobalKey popKey,
      {WavePopupDirection popDirection = WavePopupDirection.bottom,
  ...
  @override
  _WavePopupWindowState createState() => _WavePopupWindowState();
}

class _WavePopupWindowState extends State<WavePopupWindow> {
  /// targetView的位置
  ...
  /// 箭头展示方向
  late WavePopupDirection _popDirection;
        ...
        ? Positioned(
            left: widget.arrowOffset ??
                _left +
                    ...
                    widget.spaceMargin,
            top: _popDirection == WavePopupDirection.bottom
                ? _top - widget.arrowHeight
                : null,
            bottom: _popDirection == WavePopupDirection.top
                ? _bottom - widget.arrowHeight
                : null,
            child: CustomPaint(
              size: Size(15.0, widget.arrowHeight),
              painter: _TrianglePainter(
                  isDownArrow: _popDirection == WavePopupDirection.top,
  ...
  Widget _buildPopWidget() {
    ...
    return Positioned(
        ...
        top: _popDirection == WavePopupDirection.bottom ? _top : null,
        bottom: _popDirection == WavePopupDirection.top ? _bottom : null,
        child: Container(
...
typedef WavePopupListItemClick = bool Function(int index, String item);

/// popup 用于构造自定义的 Item
/// [index] Item 的索引
/// [item] Item 内容
typedef WavePopupListItemBuilder = Widget? Function(int index, String item);

/// 基于 PopUpWindow 的 弹窗列表工具类
class WavePopupListWindow {
  /// 带 itemBuilder 的 Popup List Window
  /// [popKey] 依附的组件和WavePopUpWindow组件共同持有的GlobalKey
  /// [data] 要显示的文本数据列表
  /// [popDirection] 箭头的方向
  ...
  static void showButtonPanelPopList(
    ...
    List<String>? data,
    WavePopupDirection popDirection = WavePopupDirection.bottom,
    WavePopupListItemBuilder? itemBuilder,
    WavePopupListItemClick? onItemClick,
    ...
    Navigator.push(
        ...
        WavePopupRoute(
          ...
          isShowCloseIcon: hasCloseIcon,
          offset: offset,
          widget: WaveUITools.isEmpty(data)
              ? Container(
                  constraints:
  ...
  static void showPopListWindow(context, GlobalKey popKey,
    ...
    double maxWidth = 150;
    double maxHeight = 200;
    Color borderColor = WaveThemeConfigurator.instance
        .getConfig()
        .commonConfig
        .dividerColorBase;
    Color backgroundColor = Colors.white;
  ...
  static double _getMaxWidth(TextStyle textStyle, List<String> data) {
    double maxWidth = 0;
    if (!WaveUITools.isEmpty(data)) {
      Size? maxWidthSize;
      for (String entity in data) {
        Size size = WaveTextUtil.textSize(entity, textStyle);
  ...
  static Widget _getTextWidget(WavePopupListItemBuilder? itemBuilder,
