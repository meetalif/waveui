...
class WaveSelectionSingleListWidget extends StatefulWidget {
  ...
  WaveSelectionSingleListWidget({
    ...
    /// 当前 Items 所在的层级
    currentListIndex = WaveSelectionUtil.getCurrentListIndex(
        items.isNotEmpty ? items[0] : null);
    _selectedItems = items.where((f) => f.isSelected).toList();
  }

  @override
  _WaveSelectionSingleListWidgetState createState() =>
      _WaveSelectionSingleListWidgetState();
}

class _WaveSelectionSingleListWidgetState
    extends State<WaveSelectionSingleListWidget> {
  @override
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return WaveSelectionCommonItemWidget(
              ...
              itemSelectFunction: (WaveSelectionEntity entity) {
                if ((entity.filterType == WaveSelectionFilterType.checkbox &&
                        !entity.isSelected) ||
  ...
  void configMultiLevelList(
      WaveSelectionEntity selectedEntity, int currentListIndex) {
    /// 选中【不限】清除同一级别其他的状态
    if (selectedEntity.isUnLimit()) {
