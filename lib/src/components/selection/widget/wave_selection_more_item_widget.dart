...
import 'package:waveui/src/components/picker/time_picker/date_picker/wave_date_picker.dart';
...
class WaveMoreSelectionWidget extends StatefulWidget {
  //entity 是商圈、钥匙等
  final WaveSelectionEntity selectionEntity;
  ...
  @override
  _WaveMoreSelectionWidgetState createState() =>
      _WaveMoreSelectionWidgetState();
}

class _WaveMoreSelectionWidgetState extends State<WaveMoreSelectionWidget> {
  @override
  Widget build(BuildContext context) {
    //弹出浮层
    if (widget.selectionEntity.filterType == WaveSelectionFilterType.layer ||
        widget.selectionEntity.filterType ==
            WaveSelectionFilterType.customLayer) {
...
class __FilterCommonTypeWidgetState extends State<_FilterCommonTypeWidget> {
    ...
    return GridView.count(
      ...
      children: widget.selectionEntity
        ...
        return GestureDetector(
          onTap: () {
            setState(() {
              if (data.filterType == WaveSelectionFilterType.radio) {
                ...
                data.parent?.children
                    .where((_) => _.filterType == WaveSelectionFilterType.radio)
                    .forEach((f) => f.isSelected = false);
                data.isSelected = !data.isSelected;
                //用于发送 标签点击事件
                streamController.add(SelectEvent());
              } else if (data.filterType == WaveSelectionFilterType.date) {
  ...
  Widget _buildSingleTag(WaveSelectionEntity entity) {
    bool isDate = entity.filterType == WaveSelectionFilterType.date;

    String? showName;

    if (isDate) {
      if (WaveUITools.isEmpty(entity.value)) {
...
class __MoreArrowState extends State<_MoreArrow> {
  ...
  @override
  Widget build(BuildContext context) {
    String asset = isExpanded ? WaveAsset.iconUpArrow : WaveAsset.iconDownArrow;
...
class _MoreRangeWidget extends StatefulWidget {
  ...
  ///用于自定义的筛选条件 最大值最小值
  final WaveSelectionEntity rangeEntity;
...
class __MoreRangeWidgetState extends State<_MoreRangeWidget> {
  ...
  void initState() {
    ...
    minController.addListener(() {
      if (widget.rangeEntity.filterType != WaveSelectionFilterType.range) {
        return;
      }
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.rangeEntity.filterType == WaveSelectionFilterType.dateRange) {
      return WaveSelectionDateRangeItemWidget(
...
class _FilterLayerTypeWidgetState extends State<FilterLayerTypeWidget> {
          ...
          child: GestureDetector(
            onTap: () {
              if (widget.selectionEntity.filterType ==
                  WaveSelectionFilterType.layer) {
                Navigator.of(context)
                    .push(PageRouteBuilder<WaveSelectionEntity>(
                        opaque: false,
                        pageBuilder: (context, animation, second) {
                          return WaveLayerMoreSelectionPage(
                ...
                if (widget.onCustomFloatingLayerClick != null) {
                  ...
                  widget.onCustomFloatingLayerClick!(
                      ...
                      (List<WaveSelectionEntity> customFloatingLayerParams) {
  ...
  String _getCondition() {
    ...
    //  1：滤掉不限
    //  2：滤掉有选中的非不限的叶子节点
    List<WaveSelectionEntity> result = selectedList.where((value) {
