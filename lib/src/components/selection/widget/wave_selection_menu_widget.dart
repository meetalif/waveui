...
import 'package:waveui/src/components/selection/widget/wave_selection_animate_widget.dart';
import 'package:waveui/src/components/selection/widget/wave_selection_list_widget.dart';
import 'package:waveui/src/components/selection/widget/wave_selection_menu_item_widget.dart';
import 'package:waveui/src/components/selection/widget/wave_selection_range_widget.dart';
...
typedef WaveOnRangeSelectionConfirm = void Function(WaveSelectionEntity results,
...
class WaveSelectionMenuWidget extends StatefulWidget {
  ...
  @override
  _WaveSelectionMenuWidgetState createState() =>
      _WaveSelectionMenuWidgetState();
}
...
class _WaveSelectionMenuWidgetState extends State<WaveSelectionMenuWidget> {
  ...
  WaveSelectionListViewController listViewController =
  ...
  bool _isRange(WaveSelectionEntity entity) {
    if (WaveSelectionUtil.hasRangeItem(entity.children) ||
        entity.filterShowType == WaveSelectionWindowType.range) {
      return true;
    }
  ...
  Widget _createRangeView(WaveSelectionEntity entity) {
    ...
    return WaveRangeSelectionGroupWidget(
      ...
      onSelectionConfirm: (WaveSelectionEntity result, int firstIndex,
  ...
  void _onConfirmSelect(WaveSelectionEntity entity, WaveSelectionEntity result,
      int firstIndex, int secondIndex, int thirdIndex) {
  ...
  String? _getTitle(WaveSelectionEntity entity) {
    ...
    List<WaveSelectionEntity> firstColumn =
        WaveSelectionUtil.currentSelectListForEntity(entity);
    ...
    if (firstColumn.isNotEmpty) {
      for (WaveSelectionEntity firstEntity in firstColumn) {
        secondColumn
            .addAll(WaveSelectionUtil.currentSelectListForEntity(firstEntity));
  ...
  String? _getDateAndRangeTitle(
      List<WaveSelectionEntity> list, WaveSelectionEntity entity) {
    String? title = '';
  ...
  String _getDateRangeTitle(List<WaveSelectionEntity> list) {
    ...
    if (list[0].customMap != null &&
      ...
      if (minDate != null) {
        minDateTime = DateTimeFormatter.formatDate(minDate,
    ...
    if (list[0].customMap != null &&
      ...
      if (maxDate != null) {
        maxDateTime = DateTimeFormatter.formatDate(maxDate,
            WaveIntl.of(context).localizedResource.dateFormate_yyyy_MM_dd);
      }
    }
