...
/// 左侧是：二级筛选项 右侧是三级筛选项
/// 默认将第一个元素选中
class WaveLayerMoreSelectionPage extends StatefulWidget {
  ...
  @override
  _WaveLayerMoreSelectionPageState createState() =>
      _WaveLayerMoreSelectionPageState();
}

class _WaveLayerMoreSelectionPageState extends State<WaveLayerMoreSelectionPage>
    with SingleTickerProviderStateMixin {
  List<WaveSelectionEntity> _firstList = [];

  late List<WaveSelectionEntity> _originalSelectedItemsList;

  ///当前选中的 一级筛选条件
  WaveSelectionEntity? _currentFirstEntity;
      ...
      itemBuilder: (context, index) {
        return GestureDetector(
          onTap: () {
            ...
            if (_firstList[index].filterType == WaveSelectionFilterType.radio) {
              setState(() {
                _currentIndex = index;
              ...
              _firstList[index].parent?.children.where((data) {
                return data.filterType != WaveSelectionFilterType.checkbox;
              }).forEach((data) {
  ...
  Widget _buildLeftItem(int index) {
    ...
    List<WaveSelectionEntity> list = _firstList[index].selectedList();
    //如果选中了不限 则展示 房山全部
    //如果选中了某几个
    //        如果可以跨区域 则显示数量 否则只加粗
  ...
  Widget _buildRightItem(int index) {
    bool isSingle = (_currentFirstEntity?.children[index].filterType ==
            WaveSelectionFilterType.radio) ||
        (_currentFirstEntity?.children[index].filterType ==
            WaveSelectionFilterType.unLimit);
  ...
  Widget _buildRightMultiItem(WaveSelectionEntity? entity) {
    if (entity == null) {
  ...
  Widget _buildRightSingleItem(WaveSelectionEntity? entity) {
    if (entity == null) {
      return const SizedBox.shrink();
    } else {
  ...
  void setInitialSecondShowingItem(WaveSelectionEntity currentFirstEntity) {
    //设置初始化的二级筛选条件 -1没有
    ...
    if (secondIndex == -1 && currentFirstEntity.children.isNotEmpty) {
      for (int i = 0, n = currentFirstEntity.children.length; i < n; i++) {
        if (currentFirstEntity.children[i].isUnLimit() &&
            currentFirstEntity.filterType == WaveSelectionFilterType.checkbox) {
