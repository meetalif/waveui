...
import 'package:waveui/src/components/selection/controller/wave_selection_view_controller.dart';
...
typedef WaveConfigTagCountPerRow = Function(
    int index, WaveSelectionEntity entity);

/// [menuTitle] 设置自定义 menu 的Title文案
/// [isMenuTitleHighLight] 设置自定义 menu 的 title 是否高亮
typedef WaveSetCustomSelectionMenuTitle = void Function(
    {String menuTitle, bool isMenuTitleHighLight});

typedef WaveOnSelectionChanged = void Function(
    int menuIndex,
    Map<String, String> selectedParams,
    Map<String, String> customParams,
    WaveSetCustomSelectionMenuTitle setCustomMenuTitle);
...
/// 筛选弹窗打开前的回调方法。
typedef WaveOnSelectionPreShow = WaveSelectionWindowType Function(
    int index, WaveSelectionEntity entity);

/// 点击【更多】筛选项时的回调，
/// [index] 为点击的位置，
/// [openMorePage] 为让用户触发的回调用于展开更多筛选页面
typedef WaveOnMoreSelectionMenuClick = void Function(
    int index, WaveOpenMorePage openMorePage);

/// 打开更多筛选页面，
/// [updateData] 是否要更新更多筛选的数据，
/// [moreSelections] 最新的更多筛选数据，是否更新取决于 [updateData]
typedef WaveOpenMorePage = void Function(
    {bool updateData, List<WaveSelectionEntity> moreSelections});

/// 自定义类型的 Menu 被点击时 让外部设置选中的 value 进来统一更新 UI，并将 function 传给外部设置筛选值。
typedef WaveSetCustomSelectionParams = void Function(
    Map<String, String> customParams);
...
typedef WaveOnCustomSelectionMenuClick = Function(
    ...
    WaveSetCustomSelectionParams customSelectionParams);

/// 当更多筛选页面中，类型为 CustomLayer 被回调时，该函数用于回传参数进 WaveSelectionView 中，
/// [customParams] 用户自定义参数。
typedef WaveSetCustomFloatingLayerSelectionParams = void Function(
    List<WaveSelectionEntity> customParams);

/// 类型为 CustomLayer 被点击的回调
/// [index] 被点击的位置
/// [customFloatingLayerEntity] 被点击 index 位置的筛选数据
/// [setCustomFloatingLayerSelectionParams] 外部自定义参数回传函数
typedef WaveOnCustomFloatingLayerClick = Function(
    int index,
    WaveSelectionEntity customFloatingLayerEntity,
    WaveSetCustomFloatingLayerSelectionParams
...
class WaveSelectionViewState extends State<WaveSelectionView> {
      ...
      return WaveSelectionMenuWidget(
        ...
        onMenuItemClick: (int menuIndex) {
          ...
          if (widget.originalSelectionData[menuIndex].filterType ==
                  WaveSelectionFilterType.customHandle &&
          ...
          if (widget.originalSelectionData[menuIndex].filterType ==
                  WaveSelectionFilterType.more &&
              widget.onMoreSelectionMenuClick != null) {
            widget.onMoreSelectionMenuClick!(menuIndex, (
                {bool updateData = false,
                List<WaveSelectionEntity>? moreSelections}) {
              if (updateData) {
                List<WaveSelectionEntity> moreSelectionEntities =
                    moreSelections ?? [];
        ...
        onConfirm: (WaveSelectionEntity results, int firstIndex,
  ...
  void _openMore(WaveSelectionEntity entity,
      {WaveOnCustomFloatingLayerClick? onCustomFloatingLayerClick}) {
