...
import 'package:waveui/src/utils/wave_rich_text.dart';
...
/// key-value 展示信息的集合,需要配合[WaveInfoModal]使用
///
...
///
/// 除了基本的信息展示外，使用[WaveInfoModal]还可以实现富文本、复杂Widget的功能。
/// 样式：
///    支持文本、富文本和自定义的widget
///    常用的情况可以通过类中的静态函数构造
///    富文本和Icon的情况推荐使用 [WaveRichTextGenerator] 构造
///
/// 如果不想要展开收起的功能，那么可以使用[WaveFollowPairInfo]和[WaveAlignPairInfo]
///
/// [WaveFollowPairInfo]的value紧跟随者key，只具备展示的功能
/// [WaveAlignPairInfo]的value具备对齐的功能，只具备展示的功能
///
/// ```dart
/// WavePairInfoTable(
...
///       WaveInfoModal(keyPart: "名称名称名称名称：", valuePart: "内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容"),
///      ],
/// ),
///
/// WavePairInfoTable(
...
///  * [WaveRichInfoGrid], 两列富文本展示组件
///  * [WaveAlignPairInfo], value对齐的文本组件
///  * [WaveFollowPairInfo], key-value紧紧相随的的文本组件
///
class WavePairInfoTable extends StatefulWidget {
  /// 文本信息是否对齐 默认不对齐
  final bool isValueAlign;

  /// TableCell 默认垂直对齐方式， 默认值为 [TableCellVerticalAlignment.baseline]
  /// 当 [WaveInfoModal.valuePart] 为自定义 Widget 时，可设置该参数调整对齐方式，仅在
  /// [isValueAlign] 为 true 时设置才生效
  final TableCellVerticalAlignment defaultVerticalAlignment;

  /// 待展示的文本信息集合
  final List<WaveInfoModal> children;
...
class _WavePairInfoTableState extends State<WavePairInfoTable> {
  //当前的展示状态
  late bool _isFolded;
  ...
  // 收起状态显示的孩子
  List<WaveInfoModal>? foldList;

  // 展开状态显示的孩子
  List<WaveInfoModal?>? expandedList;

  // 在页面呈现的孩子
  List<WaveInfoModal?>? showList;

  // 指定位置的最原始 modal
  WaveInfoModal? indexModal;

  // 是否具备展开收起功能 如果不展示则显示全部
  bool _canFold = false;

  late WavePairInfoTableConfig themeData;

  @override
  void initState() {
    themeData = widget.themeData ?? WavePairInfoTableConfig();
    themeData = themeData
        .merge(WavePairInfoTableConfig(rowSpacing: widget.rowDistance));
  ...
  @override
  void didUpdateWidget(WavePairInfoTable oldWidget) {
  ...
  Widget _finalValueWidget(WaveInfoModal data, {double? itemSpacing}) {
    Widget? valueWidget;
    ...
    if (data.isArrow) {
      valueWidget = Row(
        ...
        children: <Widget>[
          ...
          WaveUITools.getAssetImage(WaveAsset.iconRightArrow),
        ],
      );
    }
    return Padding(
  ...
  List<WaveInfoModal> _generateFoldList() {
    List<WaveInfoModal> finalChildren = List<WaveInfoModal>.of(widget.children);
    //生成新的value
    WaveInfoModal expRowWidget = _foldButtonWidget();
    //替换modal
  ...
  List<WaveInfoModal?> _generateExpandedList() {
    List<WaveInfoModal?> finalChildren =
        List<WaveInfoModal?>.of(widget.children);
    WaveInfoModal foldRowWidget = _expandedButtonWidget();
    finalChildren[_expandAtIndex] = indexModal;
    finalChildren[widget.children.length - 1] = foldRowWidget;
    return finalChildren;
  }

  WaveInfoModal _foldButtonWidget() {
    Image img = WaveUITools.getAssetImage(WaveAsset.iconUpArrow);
    ...
    WaveInfoModal waveInfoModal = WaveInfoModal(
    ...
    Container stack = Container(
      child: Stack(
        children: <Widget>[
          _finalValueWidget(waveInfoModal, itemSpacing: 0),
          Positioned(bottom: 0, right: 0, child: foldButtonWidget),
        ],
      ),
    );
    waveInfoModal.valuePart = stack;
    return waveInfoModal;
  }

  WaveInfoModal _expandedButtonWidget() {
    Image img = WaveUITools.getAssetImage(WaveAsset.iconUpArrow);
    Row row = Row(
      children: <Widget>[
        Padding(
  ...
  Widget _valueTitleText(String text,
      ...
      bool isArrow = false,
      required WavePairInfoTableConfig themeData}) {
    bool isSingle;
    if (isArrow) {
      isSingle = true;
    } else {
...
mixin PairInfoPart {
  ...
  WavePairInfoTableConfig configDefaultThemeData(
      WavePairInfoTableConfig? themeData,
      {double? rowDistance,
      double? itemSpacing}) {
    WavePairInfoTableConfig defaultThemeData;
    defaultThemeData = themeData ?? WavePairInfoTableConfig();
    defaultThemeData = defaultThemeData.merge(WavePairInfoTableConfig(
        rowSpacing: rowDistance, itemSpacing: itemSpacing));
    defaultThemeData = WaveThemeConfigurator.instance
  ...
  Widget? finalKeyWidget(
      WaveInfoModal data, WavePairInfoTableConfig themeData) {
    Widget? keyWidget;
  ...
  Widget finalValueWidget(WaveInfoModal data, WavePairInfoTableConfig themeData,
      {double? itemSpacing}) {
    Widget? valueWidget;
...
class WaveFollowPairInfo extends StatelessWidget with PairInfoPart {
  /// 待展示的文本信息集合
  final List<WaveInfoModal?>? children;
  ...
  @override
  Widget build(BuildContext context) {
    WavePairInfoTableConfig defaultThemeConfig = configDefaultThemeData(
        themeData,
  ...
  Widget _buildRowWidget(WavePairInfoTableConfig defaultThemeConfig) {
    int index = -1;
  ...
  Widget _buildSingleInfo(WaveInfoModal infoModal, double keyMax,
      WavePairInfoTableConfig defaultThemeConfig) {
...
class WaveAlignPairInfo extends StatelessWidget with PairInfoPart {
  /// TableCell 默认垂直对齐方式， 默认值为 [TableCellVerticalAlignment.baseline]
  /// 当 [WaveInfoModal.valuePart] 为自定义 Widget 时，可设置该参数调整对齐方式
  final TableCellVerticalAlignment defaultVerticalAlignment;
  ...
  TableRow _buildSingleInfo(WaveInfoModal infoModal, bool isLast,
      WavePairInfoTableConfig defaultThemeConfig) {
...
/// 复杂的展示 需要传入Widget，WaveInfoModal的若干静态方法 提供了丰富简便的富文本使用方式
///
class WaveInfoModal {
  ...
  /// isArrow 是否最右侧存在箭头
  static WaveInfoModal valueLastClickInfo(
    String keyTitle,
    String valueTitle,
    String clickValue, {
    double? fontSize,
    ...
    themeData ??= WavePairInfoTableConfig();
    themeData = WaveThemeConfigurator.instance
        .getConfig(configId: themeData.configId)
        .pairInfoTableConfig
        .merge(themeData);
    themeData = themeData.merge(WavePairInfoTableConfig(
        itemSpacing: itemSpacing,
        keyTextStyle: WaveTextStyle(fontSize: fontSize),
        valueTextStyle: WaveTextStyle(fontSize: fontSize)
            .merge(WaveTextStyle.withStyle(valueTextStyle)),
        linkTextStyle: WaveTextStyle(fontSize: fontSize, color: linkColor)
            .merge(WaveTextStyle.withStyle(valueTextStyle))));
    ...
    if (isArrow) {
      ...
      valueWidget = WaveRichTextGenerator()
          .addText(
            valueTitle,
  ...
  ///   /// isArrow 是否最右侧存在箭头
  static WaveInfoModal keyOrValueLastQuestionInfo(
    String keyTitle,
    String valueTitle, {
    bool keyShow = false,
    bool valueShow = true,
    ...
    themeData = themeData.merge(WavePairInfoTableConfig(
        itemSpacing: itemSpacing,
        keyTextStyle: WaveTextStyle(fontSize: fontSize)
            .merge(WaveTextStyle.withStyle(keyTextStyle)),
        valueTextStyle: WaveTextStyle(fontSize: fontSize)
            .merge(WaveTextStyle.withStyle(valueTextStyle))));
    ...
    if (isArrow) {
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              GestureDetector(
                ...
                child: WaveUITools.getAssetImage(WaveAsset.iconQuestion),
              ),
              Text(
                '：',
                overflow: TextOverflow.ellipsis,
      ...
      WaveRichTextGenerator keyGen = WaveRichTextGenerator();
      ...
      WaveRichTextGenerator valueGen = WaveRichTextGenerator();
  ...
  /// fontSize 文本
  /// isArrow 是否最右侧存在箭头
  static WaveInfoModal keyHeadIconInfo(
    String keyTitle,
    String valueTitle, {
    Widget? headIcon,
    double? fontSize,
  ...
  /// value是富文本
  static WaveInfoModal valueRichTextInfo(
    String keyTitle,
    String valueTitle, {
    double? fontSize,
    ...
    return WaveInfoModal(
        ...
        valuePart: Padding(
          ...
          child: ExcludeSemantics(
            child: WaveCSS2Text.toTextView(valueTitle,
