...
///
/// 该组件需要配合[WaveNumberInfoItemModel]使用，WaveNumberInfoItemModel是每个元素的单独模型。
///
/// 布局规则
...
///
///
class WaveEnhanceNumberCard extends StatelessWidget {
  final List<WaveNumberInfoItemModel>? itemChildren;
  ...
  Widget build(BuildContext context) {
    WaveEnhanceNumberCardConfig defaultConfig =
        themeData ?? WaveEnhanceNumberCardConfig();

    defaultConfig = defaultConfig.merge(WaveEnhanceNumberCardConfig(
  ...
  Widget _buildItemWidget(
      WaveNumberInfoItemModel model, WaveEnhanceNumberCardConfig config,
      {double? width}) {
    return Column(
  ...
  Widget _buildBottomWidget(
    ...
    if (model.iconTapCallBack != null) {
      Widget icon =
          WaveUITools.getAssetSizeImage(WaveAsset.iconQuestion, 14, 14);

      if (model.numberInfoIcon == WaveNumberInfoIcon.arrow) {
        icon = WaveUITools.getAssetSizeImage(WaveAsset.iconRightArrow, 14, 14);
      }
  ...
  Widget _getPreWidget(String? preDesc, WaveEnhanceNumberCardConfig config) {
  ...
  Widget _getLastWidget(String? lastDesc, WaveEnhanceNumberCardConfig config) {
...
class WaveNumberInfoItemModel {
  ...
  ///icon的事件
  final Function(WaveNumberInfoItemModel)? iconTapCallBack;

  ///icon的样式 可枚举 , 默认为问号
  final WaveNumberInfoIcon numberInfoIcon;
  ...
  WaveNumberInfoItemModel({
    this.number,
    this.title,
    this.numberInfoIcon = WaveNumberInfoIcon.question,
